...
local function is_protected(path)
    local p = fs_api.resolve(path)
    if p:sub(1, 6) == "/home/" or p:sub(1, 5) == "/mnt/" or p:sub(1, 5) == "/etc/" then
        return false
    end
    return true
end
...
fs_api.writeAll = function(p, d)
    if is_protected(p) then return nil, "access denied: protected from write" end
    local pr, c = getProxy(p)
    local h = pr.open(c, "w")
